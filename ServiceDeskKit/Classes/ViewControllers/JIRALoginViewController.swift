...
class JIRALoginViewController: UIViewController,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        hostLabel.text = ServiceDesk.shared.host
        projectLabel.text = ServiceDesk.shared.serviceDeskId
    ...
    @IBAction func login(){
        if let username = self.usernameField.text, let password = self.passwordField.text {
            ...
            ServiceDesk.shared.login(username: username, password: password) { (valid, error) in
